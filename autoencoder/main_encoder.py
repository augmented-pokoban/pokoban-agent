...
action_one_hot = [[0, 0, 1]]

result = tf.ones(shape=[1, 20, 20, len(action_one_hot)]) * action_one_hot

state = tf.ones(shape=[1, 20, 20, 8])

concatted = tf.concat([state, result], -1)
...
with sess.as_default():
    # print(result.eval())
    print(tf.shape(concatted))
    evalled = concatted.eval()
